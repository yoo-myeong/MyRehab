...
+ **firebase update 속도 개선**
  ...
  wemosD1에서 firebase에 update를 하거나 read를 하기 위해선 이메일 토큰 인증을 거친 뒤 진행되기 때문에 1초가 걸린다. 처음에는 LED 1 ~ 24번까지의 collection과, 압력센서 1 ~ 24번 collection을 만들어 값을 저장했다. 이렇게 할 경우 wemosD1이 한 번 루프를 돌 때 약 50초 동안 update가 진행됐다.
